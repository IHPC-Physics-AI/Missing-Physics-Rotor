...
input_file = '/content/20250521-520RPM-ringdown-1hour-scope.csv'  # Replace with your file path
...
# Initialize an empty list to store downsampled chunks
downsampled_chunks = []

# Read and process the file in chunks
for i, chunk in enumerate(pd.read_csv(input_file, chunksize=chunk_size)):
    # Take every 20th row (starting from 0)
    downsampled_chunk = chunk.iloc[::downsample_factor, :]
    # Exclude the header row from chunks after the first one
    if i > 0:
        downsampled_chunk = downsampled_chunk[1:]
    downsampled_chunks.append(downsampled_chunk)

# Combine all chunks and save to new file
downsampled_data = pd.concat(downsampled_chunks)
downsampled_data.to_csv(output_file, index=False)

print(f"Downsampling complete. Original rows: {len(pd.read_csv(input_file)):,}")
print(f"Downsampled rows: {len(downsampled_data)}")
...
file_path = '/content/downsampled_data.csv'  # Adjust as needed
...
# ---------------------------------------------------------------
window_size = 200        # seconds per FFT frame
