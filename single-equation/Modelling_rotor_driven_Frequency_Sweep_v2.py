...
for frequency in frequencies:
    ...
    solution = odeint(equation_of_motion, y0, t_eval, frequency, rtol = 1e-6, atol = 1e-3)
...
ax2 = axs[1]
ax3 = axs[1].twinx()
ax2.plot(all_time, all_theta_dot / (2 * np.pi), label="Angular Velocity (Hz)", color='orange') #divide by 2pi to convert from rad/s to Hz
ax3.plot(all_time, all_frequency, label="Frequency (Hz)", color='red', linestyle='--')
ax2.set_xlabel("Time (s)")
ax2.set_ylabel("Angular Velocity (Hz)", color='orange')
ax3.set_ylabel("Frequency (Hz)", color='red')
